    ...
    .then(csv => Papa.parse(csv, {
        ...
        transform: (value, header) => {
            switch(header) {
                ...
                case "SpAttack":
                case "SpDefense": 
                case "Speed": return Number(value);
                case "Name":
...
function getSetMonBaseStats(currentMonName: string, currentMonForm: string) {
    ...
    [baseHP.value, displayBaseHP.value] = Array(2).fill(String(currentMon.HP ?? ""));
    [baseAtk.value, displayBaseAtk.value] = Array(2).fill(String(currentMon.Attack ?? ""));
    [baseDef.value, displayBaseDef.value] = Array(2).fill(String(currentMon.Defense ?? ""));
    [baseSpA.value, displayBaseSpA.value] = Array(2).fill(String(currentMon.SpAttack ?? ""));
    [baseSpD.value, displayBaseSpD.value] = Array(2).fill(String(currentMon.SpDefense ?? ""));
    [baseSpe.value, displayBaseSpe.value] = Array(2).fill(String(currentMon.Speed ?? ""));
}
...
mainForm.addEventListener("submit", function(e) {
    ...
    statInput = (params.get("levelStats") as string).split("\n").map(level => level.split(" ").map(stat => stat !== "" ? Number(stat) : null));
    if (
        params.get("baseHP") === "" ||
        params.get("baseAtk") === "" ||
        params.get("baseDef") === "" ||
        ...
        alert("You have not set a valid Pokémon! Please set one before proceeding.")
        return;
    }
    ...
    for (const statLevel of statInput) {
        ...
        statLevels.push({
            Level: level,
            HP: (params.get("calcMode") === "exact") ? statLevel[0] : statLevels[statLevels.length - 1].HP + statLevel[0],
            Attack: (params.get("calcMode") === "exact") ? statLevel[1] : statLevels[statLevels.length - 1].Attack + statLevel[1],
            Defense: (params.get("calcMode") === "exact") ? statLevel[2] : statLevels[statLevels.length - 1].Defense + statLevel[2],
            SpAttack: (params.get("calcMode") === "exact") ? statLevel[3] : statLevels[statLevels.length - 1].SpAttack + statLevel[3],
            SpDefense: (params.get("calcMode") === "exact") ? statLevel[4] : statLevels[statLevels.length - 1].SpDefense + statLevel[4],
            Speed: (params.get("calcMode") === "exact") ? statLevel[5] : statLevels[statLevels.length - 1].Speed + statLevel[5],
            HPEV: statLevel[6] ?? 0,
    ...
    (mainFormItems["hpIV"] as HTMLOutputElement).value = (hpIV.length > 0) ? hpIV.join(", ") : "Invalid";
    (mainFormItems["attackIV"] as HTMLOutputElement).value = (atkIV.length > 0) ? atkIV.join(", ") : "Invalid";
    (mainFormItems["defenseIV"] as HTMLOutputElement).value = (defIV.length > 0) ? defIV.join(", ") : "Invalid";
    (mainFormItems["spAttackIV"] as HTMLOutputElement).value = (spaIV.length > 0) ? spaIV.join(", ") : "Invalid";
    (mainFormItems["spDefenseIV"] as HTMLOutputElement).value = (spdIV.length > 0) ? spdIV.join(", ") : "Invalid";
    (mainFormItems["speedIV"] as HTMLOutputElement).value = (speIV.length > 0) ? speIV.join(", ") : "Invalid";
});
